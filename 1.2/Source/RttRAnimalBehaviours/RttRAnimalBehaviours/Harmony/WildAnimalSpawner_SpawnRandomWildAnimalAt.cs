...
namespace RttRAnimalBehaviours
{
    ...
    public static class RaceToTheRim_WildAnimalSpawner_SpawnRandomWildAnimalAt_Patch
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator ilg)
        {
            ...
            foreach (CodeInstruction instruction in codes)
            {
                if (instruction.opcode == OpCodes.Stloc_0)
                {
                    ...
                    yield return new CodeInstruction(OpCodes.Call, typeof(RaceToTheRim_WildAnimalSpawner_SpawnRandomWildAnimalAt_Patch).GetMethod("DetectRttRCreatureAndOptions"));
        ...
        public static bool DetectRttRCreatureAndOptions(PawnKindDef theCreature)
        {
            if (theCreature != null)
            {
                ...
                if (RaceToTheRim_Mod.settings.pawnSpawnStates != null && RaceToTheRim_Mod.settings.pawnSpawnStates.Keys.Contains(theCreature.defName))
                {
                    if (RaceToTheRim_Mod.settings.pawnSpawnStates[theCreature.defName])
                    {
